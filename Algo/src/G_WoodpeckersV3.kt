...
fun main() {
    ...
    val keys = woodFriends.keys.sortedWith(compareBy ({ -woodFriends[it]!!.size}, { it } ) )
    ...
    fun dfs(root: Int, prev: Int): Int {
        ...
        if (prev == root && woodFriends[root]!!.size == 1 && woodFriends[woodFriends[root]!![0]]!!.size > 1) {
            dfs(woodFriends[root]!![0], woodFriends[root]!![0])
            return 1
        }
        isMovedIn[root] = true
        countWoodpeckers++
